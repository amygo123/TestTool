...
namespace StyleWatcherWin
{
    ...
    {
        static readonly Regex RxTitle = new Regex(@"^(?<title>.+?)(?:[:：]\s*)(?<yest>昨日[^\n]*)$", RegexOptions.Compiled);
        static readonly Regex RxSum = new Regex(@"(?<name>.+?)\s*近\s*7\s*天\s*销量\s*汇\s*总[:：]\s*(?<sum>\d+)$", RegexOptions.Compiled);
        static readonly Regex RxLine = new Regex(@"^(?<date>20\d{2}-\d{2}-\d{2})\s+(?<rest>.+?)\s*[:：]\s*(?<qty>\d+)\s*件$", RegexOptions.Compiled);
        ...
        public static ParsedPayload Parse(string raw)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    if (int.TryParse(mSum.Groups["sum"].Value, out var s)) result.Sum7d = s;
